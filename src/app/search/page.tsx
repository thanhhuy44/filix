...
async function getData(keyword: string, page: string) {
  const movies: IMovie[] = await Api.search(keyword, page);
  if (movies.length) {
    return movies;
  }
}
...
async function Page({
  ...
  const keyword: string = searchParams?.keyword as string;
  const page: string = searchParams?.page ? (searchParams.page as string) : '1';
  const movies = await getData(keyword, page);
  return (
    <main className="flex flex-col gap-y-4 sm:gap-y-6 md:gap-y-8 lg:gap-y-12 2xl:gap-y-20 my-4 sm:my-6 md:my-8 lg:my-12 2xl:my-20">
      {movies?.length ? (
        <>
          <Section
            title={`Results of "${keyword}" - Page ${page}`}
            movies={movies as IMovie[]}
          />
          <div className="flex items-center justify-center">
            <Link
              href={`/search?keyword=${keyword}&page=${parseInt(page) + 1}`}
